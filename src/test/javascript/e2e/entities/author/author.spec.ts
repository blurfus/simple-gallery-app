...
describe('Author e2e test', () => {
  ...
  it('should load Authors', async () => {
    ...
    expect(await authorComponentsPage.getTitle()).to.eq('galleryApp.author.home.title');
  ...
  it('should load create Author page', async () => {
    ...
    expect(await authorUpdatePage.getPageTitle()).to.eq('galleryApp.author.home.createOrEditLabel');
  ...
  it('should create and save Authors', async () => {
    ...
    await promise.all([
      ...
      authorUpdatePage.setEmailInput('email'),
      authorUpdatePage.resourceSelectLastOption()
    ]);
    ...
    expect(await authorUpdatePage.getEmailInput()).to.eq('email', 'Expected Email value to be equals to email');
  ...
  it('should delete last Author', async () => {
    ...
    expect(await authorDeleteDialog.getDialogTitle()).to.eq('galleryApp.author.delete.question');
